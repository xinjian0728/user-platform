        ...
        for (Controller controller : ServiceLoader.load(Controller.class)) {
            ...
            for (Method method : publicMethods) {
                ...
                if (pathFromMethod != null) {
                    fullPath = basePath + pathFromMethod.value();
                }
                handleMethodInfoMapping.put(fullPath, new HandlerMethodInfo(fullPath, method, supportedHttpMethods));
                controllersMapping.put(fullPath, controller);
            }
            //  controllersMapping.put(fullPath, controller);多个路径映射就找不到了，问题就是存在相同的对象
        }
    }
